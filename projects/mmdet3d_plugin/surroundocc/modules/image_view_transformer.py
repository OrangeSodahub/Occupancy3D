...
class LSSViewTransformer(BaseModule):
    ...
    def create_frustum(self, depth_cfg, input_size, downsample):
        ...
        H_feat, W_feat = H_in // downsample, W_in // downsample # (64, 176)
        d = torch.arange(*depth_cfg, dtype=torch.float)\
            .view(-1, 1, 1).expand(-1, H_feat, W_feat) # (80, 64, 176)
        self.D = d.shape[0]
        if self.sid:
        ...
        x = torch.linspace(0, W_in - 1, W_feat,  dtype=torch.float)\
            .view(1, 1, W_feat).expand(self.D, H_feat, W_feat) # (80, 64, 176)
        y = torch.linspace(0, H_in - 1, H_feat,  dtype=torch.float)\
            .view(1, H_feat, 1).expand(self.D, H_feat, W_feat) # (80, 64, 176)
...
class DepthNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 context_channels,  # out_channels = 64
                 depth_channels,    # D = 80
                 use_dcn=True,
                 use_aspp=True,
                 with_cp=False,
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def __init__(self, loss_depth_weight=3.0, depthnet_cfg=dict(), **kwargs):
        ...
        # in_channels=512 (out_channelf of img_fpn), out_channels=64, D=80
