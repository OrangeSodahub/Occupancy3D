...
class Metric_mIoU():
    def __init__(self,
        ...
        self.occ_xdim, self.occ_ydim, self.occ_zdim = occ_size
    ...
    def count_miou(self):
        ...
        for ind_class in range(self.num_classes-1):
            ...
            results_dict[f'{self.class_names[ind_class]} - IoU'] = round(mIoU[ind_class] * 100, 2)

        print(f'===> mIoU of {self.cnt} samples: ' + str(round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)))
        results_dict[f'mIoU of {self.cnt} samples'] = round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)
...
class Metric_FScore():
    ...
    def voxel2points(self, voxel):
        ...
        points = np.concatenate((occIdx[0][:, None] * self.voxel_size[0] + self.voxel_size[0] / 2 + self.pc_range[0], \
                                 occIdx[1][:, None] * self.voxel_size[1] + self.voxel_size[1] / 2 + self.pc_range[1], \
                                 occIdx[2][:, None] * self.voxel_size[2] + self.voxel_size[2] / 2 + self.pc_range[2]),
                                axis=1)
        return points
